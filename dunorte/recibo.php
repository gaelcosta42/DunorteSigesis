  ...
  $data_impressao = date('d/m/Y H:i:s');
	...
	*/
	...
	if($recibo58) {
		...
		if($row_venda->id_cadastro) {
			...
			if($row_venda->endereco){
				...
				if($row_venda->cidade ) {
					...
					$pdf->SetX("17");
					$pdf->SetFont('arial', 'B', 8);
					$pdf->Cell(0, 10, utf8_decode($row_venda->cidade ?? ''));
					$pdf->Ln(4);
				}
		...
		$pdf->SetX("45");
		$pdf->Cell(0, 10, (lang('DESCRICAO')), 0, 0, 'R');
		$pdf->Ln(3);
		$pdf->SetX("10");
		...
		$pdf->SetX("45");
		$pdf->Cell(0, 10, (lang('VL_TOTAL')), 0, 0, 'R');
		$pdf->Ln(2);
		$pdf->SetFont('arial', '', 7);
		$pdf->SetX("3");
		$pdf->Cell(0, 10, ("_____________________________________"));
		...
		if($voucher_crediario > 0){
			...
			$pdf->SetX("34");
			$pdf->SetFont('arial', 'B', 6);
			$pdf->Cell(0, 10, (moeda($total-$desconto-$troca)));
		}
		$pdf->Ln(5);
		$pdf->SetX("3");
		$pdf->SetFont('arial', '', 6);
		...
		if($retorno_row){
			foreach ($retorno_row as $exrow) {
				$pdf->SetX("3");
				$pdf->Cell(0, 10, (pagamento($exrow->pagamento).":"));
				$pdf->SetX("34");
		...
		if($retorno_row){
			foreach ($retorno_row as $exrow) {
				...
				$pdf->Cell(
					0,
					10,
					"Comp.: ".decimalp($exrow->comprimento)."  |  Larg.: ".decimalp($exrow->largura)
				);
				$pdf->Ln(7);
				$pdf->SetFont('arial', '', 8); 
				$pdf->SetX("10");
