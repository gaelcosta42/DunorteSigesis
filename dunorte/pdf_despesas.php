<?php
  ...
  define("_VALID_PHP", true);
	...
	$id_centro = (get('id_centro')) ? get('id_centro') : 0; 
	$valor = (get('valor')) ? get('valor') : ''; 
	$dataini = (get('dataini')) ? get('dataini') : date('d/m/Y'); 
	$datafim = (get('datafim')) ? get('datafim') : date('d/m/Y'); 
	
	
	$retorno_row = $despesa->getDespesas($id_empresa, $dataini, $datafim, $id_banco, $id_centro, $id_conta, $id_cadastro, $valor);
	$pdf = new Impressao_PDF();
	$pdf->Titulo(lang('FINANCEIRO_DESPESASAPAGAR'));
	...
	ob_start();
		require(BASEPATH . 'pdf_despesas_html.php');
