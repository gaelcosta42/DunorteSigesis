<?php
	...
	echo ' >> INICIO - '.date('d/m/Y H:i:s').'<br>';
		$sql = "SELECT p.codigobarras " 
		  . "\n FROM produto as p"
		  . "\n WHERE p.codigobarras <> '' "
		  . "\n GROUP BY p.codigobarras "
		  . "\n HAVING COUNT(1) > 1 "
		  . "\n ORDER BY p.codigobarras LIMIT 0, 100";
		...
		if($retorno_row) {
			foreach ($retorno_row as $exrow){
				...
				$primeiro = true;
				$id = 0;
				echo ' >> '.$exrow->codigobarras.'<br>';	
				$sql = "SELECT p.id, p.codigobarras " 
				  . "\n FROM produto as p"
				  . "\n WHERE p.codigobarras = '".$exrow->codigobarras."'"
				  . "\n ORDER BY p.id DESC ";
				  $nome_row = $db->fetch_all($sql);	
				if($nome_row) {
					foreach ($nome_row as $nrow){	
						$id = ($primeiro)  ? $nrow->id : $id;
						echo ' ---- >> ID: '.$nrow->id.' - ['.$nrow->codigobarras.']<br>';
						if(!$primeiro) {
