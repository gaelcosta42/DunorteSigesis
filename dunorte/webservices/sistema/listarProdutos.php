<?php
  ...
  if($searchValue != ''){
    $searchQuery = 
      " and (p.nome like '%".$searchValue."%' or 
  ...
  ## Fetch records
  $sql = "SELECT p.*, c.categoria, g.grupo, g.idcor, fa.fabricante, pp.prazo " 
      . "\n FROM produto as p"
      . "\n LEFT JOIN categoria AS c ON c.id = p.id_categoria"
      . "\n LEFT JOIN grupo AS g ON g.id = p.id_grupo"
      . "\n LEFT JOIN fabricante AS fa ON fa.id = p.id_fabricante"
  ...
  foreach($records as $row) {
    ...
    $data[] = array( 
      ...
      "cor_grupo"=>$row->idcor, 
      "produto_balanca"=>($row->produto_balanca == 1) ? 
        "<span style='background-color: #20B551; color: #fff; padding: 5px'>SIM</span>" : 
        "<span style='background-color: #f00; color: #fff; padding: 5px'>NÃO</span>"
    );
  }
