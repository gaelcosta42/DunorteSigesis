			...
			if (empty($id_unico) || strlen($id_unico)<31 || $existeVenda) {
				...
				if (!$id_cadastro and $nome_cliente) {
					$data_cadastro = array(
						...
						'cpf_cnpj' => !empty($cpf_cnpj) ? limparCPF_CNPJ($cpf_cnpj) : '',
				...
				foreach ($produtos as $prow) {
					...
					$data_cadastro_venda = array(
						...
						'valor_despesa_acessoria' => round($acrescimo / count($produtos), 2),
						'valor_desconto' => round($desconto_produto, 2),
				...
				if ($novo_valor_acrescimo->vlr_acrescimo != $acrescimo) {
					$novo_acrescimo = ($acrescimo - $novo_valor_acrescimo->vlr_acrescimo) + $novo_valor_acrescimo->valor_despesa_acessoria;
					$data_acrescimo = array('valor_despesa_acessoria' => round($novo_acrescimo, 2));
				...
				if ($novo_valor_desconto->vlr_desc != $desconto) {
					$novo_desconto = ($desconto - $novo_valor_desconto->vlr_desc) + $novo_valor_desconto->valor_desconto;
					$data_desconto = array('valor_desconto' => round($novo_desconto, 2));
				...
				foreach ($pagamentos as $parrow) {
					...
					$data_primeiro_pagamento = $parrow['data_primeiro_pagamento'];
					...
					if ($id_tipo_categoria == '1') {
						...
						$data_parcela = explode('-', $data_primeiro_pagamento);
						...
						$data_parcela = explode('-', $data_primeiro_pagamento != "null" ? $data_primeiro_pagamento : $data_venda);
						$data_parcela[2] = (strlen($data_parcela[2] > 2)) ? (int)substr($data_parcela[2], 0, 2) : $data_parcela[2];
						$valor_cheque = round($valor_pago / $total_parcelas, 2);
						$diferenca = $valor_pago - ($valor_cheque * $total_parcelas);
						$somador_data = 1;

						for ($i = 0; $i < $total_parcelas; $i++) {
							if ($data_primeiro_pagamento != "null" && $i == 0) {
								$newData = novadata($data_parcela[1], $data_parcela[0], $data_parcela[2]);
								$somador_data = 0;
							} else if ($dias == 30 || $dias == 0 || empty($dias)) {
								$newData = novadata($data_parcela[1] + ($i + $somador_data), $data_parcela[2], $data_parcela[0]);
							} else {
								$newData = novadata($data_parcela[1], $data_parcela[2] + (($i + $somador_data) * $dias), $data_parcela[0]);
							}
						...
						for ($i = 1; $i < $total_parcelas + 1; $i++) {
							if ($dias == 30) {
								$m = $i - 1;
								$newData = novadata(intval($data_parcela[1]) + intval($m), intval($data_parcela[2]), intval($data_parcela[0]));
							} else {
								$newData = novadata(intval($data_parcela[1]), intval($data_parcela[2]) + (intval($i) * intval($dias)), intval($data_parcela[0]));
							}
							$p = $i . "/" . $total_parcelas;
							$data_receita['id_banco'] = $id_banco;
