<?php
  /**
   * Webservices: Ocorrência no ponto
   *
   */
		...
		try {
			...
			if ($json_string) {
				...
				if($id_usuario){
					...
					$db->insert("ponto_ocorrencia", $data);
					if ($db->affected()) {	
						$status = "Sucesso. Ocorrencia registrada.";
						$retorno = 1;
					  }
				} else {
				$status = "Erro - ID_USUARIO invalidos.";
				}
			} else {
