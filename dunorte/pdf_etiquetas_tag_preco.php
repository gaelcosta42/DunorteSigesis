			...
			foreach($retorno_row as $exrow)
			{
				if($exrow->valor_venda > 0) {
					...
					if($coluna == '0') { // Se a coluna for 0 
						...
						$somaH = $mesq+$posicaoH+$espEti; // Soma Horizontal é a margem inicial mais a posiçãoH 
					} 
					...
					$pdf->SetFont('arial','',5);
					$pdf->Text($somaH,$somaV+5,utf8_decode($linha1));
					
					//$pdf->EAN13_sem($somaH,$somaV+7,$exrow->codigobarras, 5, 0.25);
					
					if ($exrow->codigobarras) {
						$pdf->Code128($somaH,$somaV+6,$exrow->codigobarras,25,5);
						
						//Print text uder barcode
						$pdf->SetFont('Arial','',7);
						$pdf->Text($somaH,$somaV+14,$exrow->codigobarras);
					}
					$pdf->SetFont('arial','B',7);
					//$pdf->Text($somaH,$somaV+47,utf8_decode($linha2));
					$pdf->Text($somaH,$somaV+19,utf8_decode($linha2));
					$pdf->SetFont('arial','B',5);
					$pdf->Text($somaH,$somaV+21,utf8_decode(substr($exrow->detalhamento, 0, 20)));
					$coluna = $coluna+1;
				}
			}
		}
