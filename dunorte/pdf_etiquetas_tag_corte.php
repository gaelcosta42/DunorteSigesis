<?php
   ...
   */
	...
	$leti = "35"; // Largura da Etiqueta (mm) 
	$aeti = "60"; // Altura da Etiqueta (mm)
	$ncol = "3"; // Número de colunas da folha
	$nlin = "1"; // Número de linhas da folha
	$formatoetiqueta = array(105,60);
		...
		for($q=0;$q<$quant_impressao;$q++) {
			foreach($retorno_row as $exrow)
			{
				if($exrow->valor_venda > 0) {
					...
					$pdf->Text($somaH,$somaV+3,utf8_decode($nomeempresa));
					$pdf->SetFont('arial','',6);
					$pdf->Text($somaH,$somaV+8,utf8_decode($linha1));
					
					if ($exrow->codigobarras) {
						//$pdf->EAN13_sem($somaH,$somaV+12,$exrow->codigobarras, 5, 0.25);
						$pdf->Code128($somaH,$somaV+12,$exrow->codigobarras,25,5);
						
						//Print text uder barcode
						$pdf->SetFont('Arial','',7);
						$pdf->Text($somaH,$somaV+21,$exrow->codigobarras);
						
						//$pdf->EAN13_sem($somaH,$somaV+33,$exrow->codigobarras, 5, 0.25);
						$pdf->Code128($somaH,$somaV+33,$exrow->codigobarras,25,5);
						
						//Print text uder barcode
						$pdf->SetFont('Arial','',7);
						$pdf->Text($somaH,$somaV+41,$exrow->codigobarras);
					}
					
					$pdf->SetFont('arial','B',8);
					//$pdf->Text($somaH,$somaV+47,utf8_decode($linha2));
					$pdf->Text($somaH,$somaV+45,utf8_decode($linha2));
					$pdf->SetFont('arial','B',7);
					$pdf->Text($somaH,$somaV+48,utf8_decode($exrow->detalhamento));
					$coluna = $coluna+1; 
				}
			}
		}
