...
class Sicoob {
    ...
    public function __construct() {
        ...
        $arquivoPublico = 'C:/xampp/htdocs/projetos_bitbucket/sigen1/uploads/certificados/' . SICOOBPIX_CAMINHO_CERT_PUBLICO;
        $arquivoPrivado = 'C:/xampp/htdocs/projetos_bitbucket/sigen1/uploads/certificados/' . SICOOBPIX_CAMINHO_CERT_PRIVADO;

        $this->urlAutenticacao = URL_AUTENTICACAO;
        $this->urlPix = URL_PIX;
        $this->clientId = SICOOBPIX_CLIENT_ID;
        $this->certPublico = [realpath($arquivoPublico), SICOOBPIX_SENHA_CERT];
        $this->certPrivado = [realpath($arquivoPrivado), SICOOBPIX_SENHA_CERT];
    }
    ...
    private function gerarToken() {
        ...
        $this->scope = implode(' ', ['pix.write','payloadlocation.write','pix.read','webhook.write','cob.write','lotecobv.write','cob.read','webhook.read','cobv.read','cobv.write','lotecobv.read','payloadlocation.read']);
        ...
        /*
        if ($resposta === false) {
            echo "Erro cURL: " . curl_error($curl) . PHP_EOL;
            echo "Código HTTP: " . curl_getinfo($curl, CURLINFO_HTTP_CODE) . PHP_EOL;
        } else {
            echo "Resposta da API: " . $resposta . PHP_EOL;
        }
        */
