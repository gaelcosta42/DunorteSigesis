		...
		for($q=0;$q<$quant_impressao;$q++) {
			foreach($retorno_row as $exrow)
			{
				if($exrow->valor_venda > 0) {
					$linha1 = substr($exrow->nome,0,18);
					$linha4 = substr($exrow->nome,18,18);
					$linha5 = substr($exrow->nome,36,18);
					...
					$pdf->SetFont('arial','',9);
					$pdf->Text($somaH,$somaV+9,utf8_decode($linha1));
					$pdf->Text($somaH,$somaV+12,utf8_decode($linha4));
					$pdf->Text($somaH,$somaV+15,utf8_decode($linha5));

					if ($exrow->codigobarras) {
						//$pdf->EAN13_sem($somaH,$somaV+16,$exrow->codigobarras,15,0.35);
						$pdf->Code128($somaH,$somaV+16,$exrow->codigobarras,33,15);
						
						//Print text uder barcode
						$pdf->SetFont('Arial','',8);
						$pdf->Text($somaH,$somaV+35,$exrow->codigobarras);						
					}

					if ($exrow->prazo_troca == 0 || $exrow->prazo_troca == '') {
						$pdf->Text($somaH,$somaV+38,utf8_decode('Prazo de troca 7 dias.'));
					} else {
						$pdf->Text($somaH,$somaV+38,utf8_decode('Prazo de troca '.$exrow->prazo_troca.' dias.'));
					}

					$pdf->SetFont('arial','B',15);
					//$pdf->Text($somaH,$somaV+47,utf8_decode($linha2));
					$pdf->Text($somaH,$somaV+55,utf8_decode($linha2));
					$pdf->SetFont('arial','B',8);
					$pdf->Text($somaH,$somaV+60,utf8_decode($exrow->detalhamento));
					$coluna = $coluna+1; 
				}
			}
		}
