...
$condicoes = '';
if (!empty($pesquisa)) {
    $condicoes .= " AND (observacao LIKE '%$pesquisa%' OR id = '$pesquisa' OR csosn_cst = '$pesquisa' OR csosn = '$pesquisa')";
}
...
$qry = "SELECT  id, csosn_cst, csosn, observacao
        FROM conversao_csosn
		WHERE inativo = 0
        $condicoes
        ORDER BY csosn_cst ASC
...
if ($stmt->execute()) {
    ...
    while ($row = $resultado->fetch_assoc()) {
        ...
        $linhas[] = array(
            ...
            $csosn,
            $observacao,
            "<button type='button' data-fluxo='csosn' data-codigo='$id' class='btn blue atualizar'><i class='fa fa-pencil' aria-hidden='true'></i></button>
            <button type='button' data-fluxo='csosn' data-acao='inativar' data-codigo='$id' class='btn red inativar'>X</button>"
        );
    }
