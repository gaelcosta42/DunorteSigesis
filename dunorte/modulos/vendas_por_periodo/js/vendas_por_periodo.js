...
$(document).ready(function () {
    let tabela_dinamica = new SimpleTable(null, `#dynamic-table`, `#pesquisa-dinamica`, true, 'modulos/vendas_por_periodo/load/load_vendas_por_periodo.php', '.payload');

    $(document).on('retornoChanged', function (event, newValue) {
        if (newValue != null && $("#valorTotal").val() != `${newValue}`) {
            $("#valorTotal").html(`${newValue}`);
        } else {
            if (tabela_dinamica.paginaAtual == 0) {
                $("#valorTotal").empty();
            }
        }
    });
    ...
    $(document).on('click', '#selecionardata', function () {
        if ($('#dataini').val() != "" && $('#datafim').val() != "") {
            ...
            Swal.fire({
                title: "Atenção!",
                text: "Selecione as datas de início e fim do período para pesquisar.",
                icon: "warning"
            });
        }
    });
    ...
    $(document).on('click', '#copiar', function () {
        ...
        if (!elemento.is(':disabled')) {
            ...
            $.ajax({
                type: "POST",
                url: "modulos/vendas_por_periodo/load/load_vendas_por_periodo_copy.php",
