									...
									<tr>
										<td>
											<table class="table table-bordered table-advance table-condensed">
												<thead>
													<tr>
														<th colspan="2"><?php echo lang('ADIANTAMENTO_SALARIAL');?></th>
													</tr>
												</thead>
												<tbody>
													<tr>
														<th><?php echo lang('MES_ANO');?>:&nbsp;&nbsp;</th>
														<td><?php echo ($data) ? exibeMesAno($data, true, true) : date("d/m/Y H:i");?></td>
													</tr>
												<tbody>
											</table>
										</td>
											...
											<table class="table table-bordered table-striped table-advance table-condensed">
												<thead>
													<tr>
														...
														<th><?php echo lang('FUNCAO');?></th>
														<th><?php echo lang('VALOR');?></th>
													</tr>
												</thead>
												<tbody>
												<?php 	
													$retorno_row = $financeiro->getAdiantamentos();
													$total = 0;
													if($retorno_row):
													foreach ($retorno_row as $exrow):
													$valor = $exrow->salario * 0.4;
													if($exrow->nivel == 4) {
														$valor = $exrow->salario;
														$horas = $financeiro->getHorasInstrutor($exrow->nome, $data);
														$valor = $valor * $horas;
													}
													$total += $valor;
												?>
													<tr>
														...
														<td><?php echo ($exrow->nivel == 4) ? "INSTRUTOR" : "COLABORADOR";?></td>
