...
require_once("fpdf/fpdf_carta.php");
require_once("fpdf/rotation.php");
require_once("init.php");

class Impressao_Carta extends FPDF_rotation  {

	private $font 			 = 'helvetica';	 	/* Font Name : See inc/fpdf/font for all supported fonts */
	private $columnOpacity   = 0.00;		 	/* Items table background color opacity. Range (0.00 - 1) */
	...
	private $margins 		 = array('l' => 15,
									 't' => 5,
									 'r' => 15,
									 'b' => 15,); /* l: Left Side , t: Top Side , r: Right Side */
	...
	public $addText;
	public $footernote;
	public $dimensions = array(0,0); /*FF*/
    ...
    ******************************************/
	public function __construct($size='A4',$currency='R$',$language='pt-BR') {
		$this->columns  		  	= 3;
		$this->items 			  	= array();
		$this->totals 			  	= array();
		$this->addText 			  	= array();
	...
	public function novaPagina($size='A4',$currency='R$',$language='pt-BR') {
		$this->columns  		  	= 3;
		$this->items 			  	= array();
		$this->totals 			  	= array();
		$this->addText 			  	= array();
	...
	public function addItem($item,$description = "",$quantity,$vat,$discount = 0,$total) {
		$p['item'] 			= $item;
	...
	public function Header() {
		...
		if($this->PageNo()== 1) {
			...
			$this->Cell(0,5,iconv("UTF-8", "ISO-8859-1",$this->title),0,1,'L');
			...
			if($this->display_tofrom === true) {
				...
				$this->Ln(3);
				$count_from = is_array($this->from) ? count($this->from) : 0;
				$count_to = is_array($this->to) ? count($this->to) : 0;
				for($i=1; $i<max($count_from,$count_to); $i++) {
					$this->Cell($width,$lineheight,iconv("UTF-8", "ISO-8859-1",substr($this->from[$i],0,50)),0,0,'L');
					$this->Cell(0,$lineheight,iconv("UTF-8", "ISO-8859-1",substr($this->to[$i],0,50)),0,0,'L');
					$this->Ln(5);
				}	
				$this->Ln(-6);
				$this->Ln(5);
			...
			$this->SetFont($this->font,'B',9);
			$this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
			$this->Cell(40,0,iconv("UTF-8", "ISO-8859-1",strtoupper($this->lang['date'])).':',0,0,'L');	
			$this->SetTextColor(50,50,50);
			$this->SetFont($this->font,'',9);
			$this->Cell(0,0,$this->date,0,1,'L');
	...
	public function Body() {
		...
		if($this->items) {
			foreach($this->items as $item) 
			{
				...
				{
					//Precalculate height
					$calculateHeight = new Impressao_Carta;
		...
		if($this->totals) 
		{
			foreach($this->totals as $total) 
			{
				...
				$this->Cell($width_other-1,$cellHeight,iconv('UTF-8', 'windows-1252',$total['name']),1,0,'L',1);
				$this->Cell($this->columnSpacing,$cellHeight,'',0,0,'L',0);
				$this->SetFont($this->font,'b',8);
				$this->SetFillColor($bgcolor,$bgcolor,$bgcolor);
				if($total['colored']) 
				...
				$this->Cell($width_other,$cellHeight,iconv('UTF-8', 'windows-1252',$total['value']),1,0,'C',1);
				$this->Ln();
				$this->Ln($this->columnSpacing);
			}
		}
	...
	public function Footer() {
		...
		// Page number
		$this->Cell(0,10,('SIGESIS - ').date('d/m/Y H:i:s'),0,0,'L');
